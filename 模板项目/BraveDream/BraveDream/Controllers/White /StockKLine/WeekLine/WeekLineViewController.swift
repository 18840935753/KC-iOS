...
class WeekLineViewController: BaseLineViewController {
    ...
    // 月数据
    private func netWeekLineData() {
        
        let params = ["code": "000001",
                      "adjusted": "",
                      "type": "14901",
                      "from": Util.date(before: 365*6),
                      "to": Util.date(before: 0)]
        
        showIndicator(to: kLine)
        vm.postWeekLineData(params, success: { (data) in
            self.weekLineData = data as? JSUKLineMessage
    ...
    // 月K数据
    private var weekLineData: JSUKLineMessage? {
        didSet {
            guard let model = weekLineData else { return }
